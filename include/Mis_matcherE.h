...
private:
	...
	void InitalizeNiehbors(Mat &neighbor, const Size& GridSize) {
		for (int i = 0; i < neighbor.rows; i++)
		{
			...
			memcpy(data, &NB9[0], sizeof(int) * 9);//9
		}
	}
	...
	void SetScale(int Scale) {
		...
		mGridNeighborRight = Mat::zeros(mGridNumberRight, 9, CV_32SC1);//9
...
int gms_matcher::GetInlierMask(vector<bool> &vbInliers, bool WithScale, bool WithRotation) {
	...
	{
		for (int Scale = 0; Scale < 5; Scale++)//5
		{
			SetScale(Scale);
